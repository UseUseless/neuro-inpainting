"""Визуальная валидация работы детектора (YOLO-Seg).

Скрипт загружает обученную модель и запускает инференс на тестовой выборке изображений.
Основная цель — визуально проверить точность сегментации. Скрипт генерирует изображения,
где найденная ватермарка залита полупрозрачным красным цветом, что позволяет мгновенно
оценить качество покрытия (Coverage) и точность границ.
"""
